...
public class PunettSquare extends JPanel{
  ...
  public String sortPhenos(String p){
    String re;
    String r = p.charAt(1) + "" + p.charAt(3)+"";
    String b = p.charAt(0) + "" + p.charAt(2) + "";
  ...
  public char[] sortCharPheno(char[] g){
    char[] re;
    String r = g[2] + "" + g[3];
    String b = g[0]+ "" + g[1];
    if(r.equals("rR"))
    ...
    if(b.equals("bB"))
      b = "Bb";
    re = new char[]{b.charAt(0), b.charAt(1), r.charAt(0), r.charAt(1)};
    return re;
  }
  ...
  public void paintComponent(Graphics g){
    ...
    g2d.drawString(new String(sortCharPheno(creatures[0].getGeno().getGenotypes())),
                   1, txtH*2);
    txtW = metrics.stringWidth(new String(creatures[0].getGeno().getGenotypes()));
    g2d.drawString(" X " + new String(sortCharPheno(creatures[1].getGeno().getGenotypes())),
                   txtW + 10, txtH*2);
    ...
    for(int c = 0; c < 4; c++){
      for(int r = 0; r < 4; r++){
        g2d.drawString(sortPhenos(newPheno[count]), (getWidth()-IWIDTH)+(2 + IGRIDSIZE * (r)),(getHeight()-IHEIGHT)+(phenoH+ IGRIDSIZE * (c)));
        count++;
      }
    }
    g2d.setColor(Color.RED);
